...
// We create the third tile layer that will be the dark background of our map.
...
let majorData = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson";
  ...
  d3.json(majorData).then(function(data) {
    ...
    // Change the getColor() fuction for major earthquakes.
    function getColor(magnitude) {
      if (magnitude > 5) {
        return "#ea2c2c";
      }
    ...
    // Change the getRadius() fuction for major earthquakes.
    function getRadius(magnitude) {
      if (magnitude === 0) {
        return 1;
      }
  ...
  // Create a GeoJSON layer with the retrieved data that adds a circle markers.
  // Then set the style of the marker, and display the magnitude and location of earthquake.
  L.geoJson(data, {
    pointToLayer: function(feature, latlng) {
      return L.circleMarker(latlng);
    },
